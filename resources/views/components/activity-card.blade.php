...
<a href="{{ route('activities.show', $activity) }}" class="group card card-hover activity-card flex flex-col overflow-hidden">
    ...
    <div class="flex flex-1 flex-col px-4 py-3.5">
        ...
        <h3 class="mt-2 font-display text-[1.15rem] leading-snug text-forest">{{ $activity->title }}</h3>

        @if (filled($activity->excerpt))
            <p class="mt-1.5 line-clamp-2 text-[13px] leading-relaxed text-muted">{{ $activity->excerpt }}</p>
        @endif
