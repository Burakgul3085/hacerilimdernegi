...
<a href="{{ $album->publicUrl() }}" class="group card card-hover flex flex-col overflow-hidden">
    <x-cover :src="$album->cover" :alt="$album->title" ratio="aspect-[4/3]" />

    <div class="flex flex-1 flex-col p-6">
        <p class="tag">{{ $album->cardLabel() }}</p>
        <h3 class="mt-2 font-display text-2xl leading-snug text-forest transition duration-300 group-hover:text-gold">{{ $album->title }}</h3>
